...
class MagneticCoordinator: Coordinator {
    ...
    func start() {
        presentViewController(makeMagneticController())
    }
...
extension MagneticCoordinator {
    private func makeMagneticController() -> UIViewController {
        let speedTestViewController = MagneticController(coordinator: self)
        return speedTestViewController
    }
}

extension MagneticCoordinator: PaywallViewControllerDelegate {
    func presentPaywall() {
